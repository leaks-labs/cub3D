...
t_mlx_exception	ft_init_graphx(t_game_data *game_data, t_graphx *graphx)
{
	...
	mlx_hook(graphx->window.mlx_win, ON_KEYDOWN, (1L << 0), \
				&ft_key_handler, game_data);
	graphx->mouse_tracked = false;
	mlx_hook(graphx->window.mlx_win, ON_MOUSEMOVE, 0, \
				&ft_on_mouse_move, game_data);
